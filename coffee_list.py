...
if logged_in == True and profile_nav == "Show diagrams":
    ...
    if correlation:
       ...
       fig5 = px.scatter(df, x='x-values', y='y-values', size='size', hovertemplate='<br>%{x} with %{y}<br>' + '%{size}', labels={"x-values":"", "y-values":""}, title="Absolute correlation", color='size')#, text='size')
